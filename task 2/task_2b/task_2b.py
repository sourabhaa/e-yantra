...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	vision_sensor = np.array(vision_sensor_image,dtype = np.uint8)
	vision_sensor.resize([image_resolution[0],image_resolution[1],3])
	rgb_image = cv2.cvtColor(vision_sensor,cv2.COLOR_BGR2RGB)
...
def send_data(maze_array):
	...
	##############	ADD YOUR CODE HERE	##############
	inputInts=maze_array
	inputFloats=[]
	inputStrings=[]
	inputBuffer=[]
	return_code = sim.simxCallScriptFunction(client_id,"Base",sim.sim_scripttype_customizationscript,"receiveData",inputInts,inputFloats,inputStrings,inputBuffer,sim.simx_opmode_oneshot)
